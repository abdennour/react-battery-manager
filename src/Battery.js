...
class Battery extends React.Component {
  ...
  onCharging(event) {
    this.setState({level : event.target.level * 100, charging:true}) ;
  }
  onDisCharging(event) {
    this.setState({level : event.target.level * 100, charging:false}) ;
  }
  removeCss() {
  ...
  getContainerSize() {
    ...
    if(this.props.width && this.props.height ) {
       ...
       containerHeight = this.props.height;
        ...
        let sizeMap = {small:[36, 12] , large: [72, 24], xlarge: [108, 36], xxlarge: [144, 48]};
        containerWith = sizeMap[this.props.size][0];
        containerHeight = sizeMap[this.props.size][1];

     }

     return {width: containerWith, height: containerHeight, padding: parseInt(containerHeight/12)};
  }
  getForeground() {
    return (!this.state.charging)?this.props.foreground :this.props.colorOnCharing;
  }
  getContainerColors() {
    return {background : this.props.background, border: `1px solid ${this.getForeground()}` }
  }
  ...
  getBarStyle() {
    return {width: `${this.state.level}%`, background: this.getForeground()};
  }
  ...
  render() {
   ...
   return (
    <div className={"battery_"+this.state.id} style={this.getContainerStyle()}>
     <div className={"level_"+this.state.id} style={this.getBarStyle()}>
     </div>
    </div>
    )
  }
